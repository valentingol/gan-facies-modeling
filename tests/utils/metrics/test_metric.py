...
def test_wasserstein_distances(capsys: Any) -> None:
    ...
    wasserstein_distances(indicators_list_1,
                          ...
                          connectivity=None,
                          unit_component_size=1)
    wasserstein_distances(indicators_list_1[:-1],
                          indicators_list_2,
                          connectivity=None,
                          unit_component_size=1)
...
def test_evaluate() -> None:
    ...
    os.makedirs('tests/datasets', exist_ok=True)
    np.save('tests/datasets/data32.npy', data32)
    gen = SAGenerator(n_classes=3,
                      model_config=config32.model)
...
def test_print_metrics() -> None:
    """Test print_metrics."""
    metrics = {'ind1_cls_1': 0.3, 'ind2_cls_1': 0.2,
               'ind1_cls_2': 0.1, 'ind2_cls_2': 0.4,
               'global': 0.5, }
